...
function Get-Mailbox_LinkedMasterAccounts
{
    ...
    foreach($MailBox in $ClientMailBoxes)
    {
        ...
        if ($MailBox.LinkedMasterAccount)
        {
            ...
            if(-not($MailBox.EmailAddresses -match $ADAccount.UserPrincipalName))
            {
                ...
                #$script:GroupBy = $Report | Group-Object -Property $ClientOU

                #$GroupBy | get-member
                #$GroupBy.Group
                #$GroupBy.Name
                #$GroupBy.Count
                #$GroupBy -is [system.Array]
    ...
    #write-host $GroupBy.Group


    $GroupBy = $Report | Group-Object -Property $ClientOU
    $GroupBy = $GroupBy.Group
  
    $GroupBy.Group -is [array]

    #write-host "Test:" $GroupBy

    foreach($Group in $GroupBy)
    {
        #$GroupBy | get-member
        
        #Write-Host "${Group}: $(#GroupBy.Item($Group))"

        #write-host "$Group.Key $Group.item"
        write-host "Name:" $Group.MailBox_Name
        #write-host "Name:" $Group.Name
        #write-host "Name:" $Group.Group
