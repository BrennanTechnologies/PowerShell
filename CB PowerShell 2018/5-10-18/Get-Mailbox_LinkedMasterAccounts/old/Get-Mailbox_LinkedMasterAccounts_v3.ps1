...
function Import-ExchangeSession
{
    ...
    if (-not $ExchangeSession) 
    {
        ...
        $ExchangeCAS = (HostName).split("-")[0] + "-htcas01.ecilab.net"
    ...
    foreach($MailBox in $ClientMailBoxes)
    {
        ...
        if ($MailBox.LinkedMasterAccount)
        {
            ...
            try
            {
                ...
                $ADAccount = Get-ADUser -Server $Domain -Identity $UserName -Properties Name,SamAccountName,GivenName,Surname,EmailAddress,Mail,Displayname,DistinguishedName -ErrorAction silentlycontinue
            ...
            if(-not($MailBox.EmailAddresses -match $ADAccount.UserPrincipalName))
            {
                ...
                write-host "EX_MailBox: " $MailBox.EmailAddresses
                write-host "AD_samAccountName: " $ADAccount.samAccountName
                write-host "AD_Name: " $ADAccount.Name
                write-host "AD_UPN: " $ADAccount.UserPrincipalName
                write-host "AD_Mail: " $ADAccount.mail
                #exit
