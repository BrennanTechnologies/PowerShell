    ...
    foreach($MailBox in $ClientMailBoxes)
    {
        ...
        if ($MailBox.LinkedMasterAccount)
        {
            ...
            function OldRemediate-Mailbox
            {
                ...
                $Results = "What if: Setting on Target 'Exchange MailBox.DistinguishedName' : " + $Target + "`tUpdating with: 'AD Account.UserPrincipalName' : " + $ADAccount.UserPrincipalName
                $Results
            ...
            function Remediate-Mailbox
            {
                ...
                $Results = "What if: `nTARGET:   " + $Target + "    `nVALUE:   " + $Value
               
                $WhatifFile = $ScriptPath + "\Whatif.txt"
                [string]$Command = ((($Scriptblock -replace 'Target', $Target) -replace 'Value', $Value) -replace [char]36,$NULL)
                $Command = $Command.Replace('$','')

                if ($WhatIfPreference)
                {
                    write-host "Whatif is True: Running Whatif Logic ...." -ForegroundColor Green
                    write-host $Results -ForegroundColor Magenta
                    $Command | Out-file -FilePath $WhatifFile -Append -Force -Whatif:$false
                    $Results | Out-file -FilePath $WhatifFile -Append -Force -Whatif:$false
                }
                elseif ($Commit)
                {
                    write-host "Validated: Commit Flag is True"
                    if($Value -IsNot [System.Array] -and $Value.count -eq 1) 
                    {
                        write-host "Validated: Target is a Single Object"
                        
                        write-log "Results: $Results" yellow
                        write-log "Command: $Command" yellow
                        #write-log "Target: $Target" yellow
                        #write-log "Value: $Value" yellow
                        
                        write-host "Undo Command" -ForegroundColor Cyan
                        
                        $Undo = Set-Mailbox -identity "Ham Burglar" -EmailAddresses @{remove="Ham@macdonaldventures.com"}


                        #$Scriptblock = $Scriptblock.ToString() + " -confirm"
                        #$scriptblock
